...
func (r *outboxRepository) LockPending(ctx context.Context,limit int) ([]model.OutboxEvent, error) {
	...
	err := r.db.
		...
		Where(
           "status = ? AND (next_retry_at IS NULL OR next_retry_at <= NOW())",model.OutboxPending,
        ).
...
func (r *outboxRepository) IncrementRetry(ctx context.Context,id uuid.UUID,nextRetry time.Time,lastError string) error {
